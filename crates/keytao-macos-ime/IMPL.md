...
2. 修正 `commitComposition` 的 Return keyval，让它和普通按键路径一样使用 `XK_Return`/`0xff0d`。
3. 为 macOS 增加 IME 专用日志入口，并接入 App `read_debug_logs` 或新的平台日志命令。
4. 增强 `macos_ime_status`：检查 bundle、TIS 注册、主输入源 enabled/selectable/current、旧输入源残留。
5. 明确 CapsLock 策略：要么去掉 `TICapsLockLanguageSwitchCapable` 声明，要么实现 flagsChanged 同步和 Rime switch_key 对齐。
6. 再补 schema 切换、选项开关、周边文本、鼠标 hover/滚轮等体验能力。
...
1. 清理安装/卸载过渡接口  
   后端仍有 `macos_install_ime` 和 `macos_uninstall_ime`，但正式产品 UI 已不暴露这些操作。输入法应跟随应用 pkg 安装、升级、卸载；后续可以把这些命令收敛为开发模式命令或移到独立维护脚本，简化用户路径。
