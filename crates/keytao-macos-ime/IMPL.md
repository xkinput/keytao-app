...
6. `ensureEngineReady()` 在 runtime 尚未就绪时调用 `initializeEngine()`，因此先启动输入法、后在 App 部署也可以重试。
7. `initializeEngine()` 解析 user/shared dir 后调用 `keytao_init(userDir, sharedDir)`；该入口只加载已部署产物，未安装或未部署时会失败并等待后续重试。
...
2. 隐藏候选窗和模式提示。
3. 销毁当前 Rime session。
4. 调用 `keytao_reload()`，由通用 runtime 重新加载 App 已部署的产物并递增 generation。
5. 创建新 session。
...
6. `asciiMode && !hasComposition` 时直接放行，让系统输入英文；`F4` 例外，仍送入 librime 打开 Rime 菜单。
7. Carbon 特殊键转换为 X11 keysym：Return、Backspace、Delete、Escape、Space、方向键、Home/End、PageUp/PageDown、Tab、F4。
