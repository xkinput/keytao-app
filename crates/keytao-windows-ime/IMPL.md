...
3. `TextService::Activate` 保存 `ITfThreadMgr` / `client_id` 并注册 `ITfKeyEventSink`，然后只启动后台 engine warmup，不等待 librime。
4. 后台 warmup 在不持有 `TsfState` mutex 的情况下创建 `keytao_core::ImeRuntime`，调用 `init()` 并创建 `ImeRuntimeSession`，完成后只用短锁安装到共享状态中。
5. warmup 未完成或 reload 正在运行时，`OnTestKeyDown` / `OnKeyDown` 直接放行按键，不阻塞 TSF 输入线程。
