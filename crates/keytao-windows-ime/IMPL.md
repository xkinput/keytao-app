...
- “中/英”右键提供“重新部署”和“打开 App”。保持 `BTN_BUTTON`，在 `OnClick` 中显示原生菜单；菜单使用独立的临时 owner，避免向宿主发送 `WM_COMMAND`，且不持有状态借用进入菜单消息循环。
- 菜单操作通过持有 DLL activity guard 的后台线程打开固定的 `keytao://ime/open` 或 `keytao://ime/redeploy` URI。NSIS 注册带引号的协议命令；主程序使用 Windows single-instance 插件唤醒已有窗口，并将部署请求保留至前端就绪。重新部署复用主程序的进度、错误提示及现有安全部署流程，输入线程不执行部署。
...
- 候选面板定位结果区分 `CaretSource::Probe / Cache / System / Accessibility / View / Window` 来源；只有未裁剪且位于 owner monitor 内的 fresh Probe 会写入 `last_caret` 并停止重探，Cache/System 仅作临时显示，不能冒充新探测。每次 `apply_ime_state` 只允许启动一次 `16/32/64/128/256 ms` 退避链，总计最多 5 次；Probe 成功停止当前链，layout 回调不能重置预算或重启链。`OnLayoutChange` 只对可见面板当前 context 的 `TF_LC_CHANGE` 安排独立的 16 ms 单次 timer，同一 burst 合并成一次只读重探。系统 caret 只通过 `GetGUIThreadInfo(GetCurrentThreadId())` 获取，并拒绝非同一 root window、退化矩形和 client origin；TSF 与系统 caret 失败后，在文档锁外通过当前线程焦点窗口的 `OBJID_CARET` / `IAccessible::accLocation` 查询自绘光标，调用前后校验窗口归属与焦点，拒绝重入、零高及无效矩形。若仍没有真实 caret 且存在候选内容，则在可见编辑视口（`GetScreenExt` 与 owner client 交集，必要时 owner client）左上内侧显示候选；`View/Window` 明确表示降级位置，不写 caret cache、不停止重探、不用于中英模式提示。所有定位仍受窗口可见性及屏幕边界约束，后续有效 TSF 布局优先恢复光标跟随。
...
- composition 必须在 TSF edit session 里用 `ITfContextComposition::StartComposition` 创建，用 `ITfRange::SetText` 更新 range，用 `ITfComposition::EndComposition` 结束。非嵌入模式在宿主不提供有效 caret 时保留独立的空 `PanelAnchor` composition，触发 TSF/IMM 的原生组字与布局协议；提交前只结束 anchor，再在同一个 write session 内从当前 QUERYONLY insertion range 创建文本 composition、写入提交文本并结束。嵌入模式可复用 active composition；两种模式都不存在 `TF_IAS_NOQUERY` 裸写路径。
...
1. `OnTestKeyDown` 先透传应用快捷键，再按上下文短路（见下），最后调用 `should_eat_key()` 告诉 TSF 是否拦截，同时维护 solo Shift 的 pending 标志。直接调用 `OnKeyDown` 的宿主也执行相同的快捷键透传。
...
5. 不论是否正在组字，Ctrl / Alt / Win 组合以及 Shift+Insert、Shift+Delete、Shift+F4 都在进入 Rime 前交给宿主。只保留输入法的 Ctrl+grave、Ctrl+Shift+grave、Ctrl+Shift+1..5 和裸 F4；附加 Alt/Win 时仍透传。这样 Ctrl+V 不再被默认 Emacs 绑定当作候选翻页，Ctrl+A 不再变成预编辑 Home，AltGr 字符也交由宿主布局处理。没有 composition 时，裸 Space、Return、Backspace、Delete、Tab、Escape 和导航键继续放行。
...
8. `should_eat_key()` 仅对普通可打印输入保持宽松判定，让 Rime 的 punctuator 收到标点；`OnKeyDown` 的 `should_consume_processed_state()` 根据 `accepted`、commit 或可见组字状态变化决定是否消费。应用快捷键不走这条“先声明拦截、随后放行”的路径：WPF 等宿主在 Test 阶段就改变快捷键路由，而 Rime 也可能真正接受冲突绑定。中文标点仍由 Rime 处理，Ctrl+C/V/X 等从两个入口直接透传。
...
10. `plan_composition(state, embedded)` 决定正文 preedit：嵌入模式把非空 preedit 写进 composition range；非嵌入模式返回 `CompositionTarget::None`，但定位失败时可以创建独立且不含文字的 `PanelAnchor`。
...
`OnKeyUp` 只处理 solo Shift：先发送 `Shift_L` 或 `Shift_R` keysym 的 press（mask `0`），再发送同一 keysym 的 release（`RIME_RELEASE_MASK`）。librime 的 ascii composer 必须先看到 press 才会在 release 上切换；两次调用都在真实 `OnKeyUp` 内，`OnTestKeyDown` 始终不调用 librime。如果 Shift 按下后出现其它 keyDown，或同时按着 Ctrl/Alt/Win，pending flag 会被清除；两个 release 入口也复核组合修饰键，避免 Ctrl+Shift 触发额外中英切换。Shift keydown 仍向宿主透传。

release 即使返回 `accepted=false`，只要 `ascii_mode` 改变也必须 apply；返回宿主的 eaten 仍是原来的 `consumed`，不因模式变化而吞掉 keyup。只有无 commit、模式改变且没有可见 preedit / 候选的 mode-only apply 可以在 composition 为空时保留有效 caret cache；普通非嵌入 apply 或任何 commit 都清除旧 cache。新 probe 不可用时，模式提示使用可验证的上次 caret / system caret；完全无坐标时省略提示，但照常更新 `st.ascii_mode` 与语言栏。

Caps Lock 本批保留宿主原有路径，不新增 `VK_CAPITAL` keysym 或拦截规则。vendored `ascii_composer.cc::ProcessCapsLock` 在 press 上立即切换并可能提交，且明确区分 Windows 与 IBus 的 lock modifier 时序；简单复用 Shift 的“丢弃 press 返回值、处理 release”会丢失 press commit，mask `0` 也不能表达原生 Caps Lock 状态。这是 F1 允许保留的例外，不能宣称 Caps Lock schema 切换已修复。
...
- 探测结果区分 `ContextProbe::Restricted` / `Clear` / `Unknown` / `Unavailable` / `ProbeFailed`。属性值、selection 或 compartment 形态无效仍为 `Unknown` 并阻挡输入。Word 的可选 input-scope `GetValue` 在有效 selection 上返回 `E_FAIL` 时记为 `Unavailable`，同步只读 session 被拒绝时记为 `ProbeFailed`；这两种结果本身不等于宿主禁用输入。同一个 COM context 保留上次已知的 `Clear` / `Restricted`；此前的 `Restricted` 在后续 `Unknown` 下也保留，直到明确的新答案或 context 切换。未曾声明敏感的上下文可继续组字，但这不表示失败的探测已证明它不是密码字段。
- `Unknown`、`Unavailable` 与 `ProbeFailed` 都需要重试。`OnTestKeyDown` / `OnKeyDown` 在判断是否放行**之前**先跑 `retry_input_context_if_unknown()`，未解答或仍在执行的 probe 从上一次真实尝试起至少间隔 250 ms，包括 focus 入口。同一 context 连续 3 次 `TS_E_SYNCHRONOUS` 后退避到 1 s；focus / compartment churn 不能清除尝试时间戳，等待期间只刷新两个 compartment。换 context 清同步拒绝计数但保留时间戳，未探测的新 context 保持 `Unknown`；上次成功解答后新焦点可立即探测。重试必须放在测试回调里——TSF 对测试回调拒绝的按键不会调 `OnKeyDown`。
...
Windows 默认使用非嵌入模式。设置保存在 `%APPDATA%/keytao/theme.yaml` 的 `ui.embeddedComposition`，兼容别名 `embedded_composition`；文件缺失、key 缺失和新安装都解析为 `false`。TIP 通过进程级 `ThemeResolver` 和 `OnceLock` / `Mutex` 的 250 ms theme cache 跟随文件签名变化，不把 resolver 放入 `TsfState`。`apply_ime_state` 每次只读一次模式并保存有效值到 `embedded_mode_cached`；reposition 只消费该缓存，不再重新检查主题文件。主 App 的「输入法外观」卡片只在 Windows 显示开关，并由桌面命令 `set_ime_embedded_composition` 原子更新 YAML、写 reload stamp。

- 非嵌入模式不把 preedit 写入宿主文档。若普通 TSF 定位失败，以 `PanelAnchor` 建立不含文本的原生 composition；panel 的 `panel_context` 独立记录最后一次显示上下文。候选窗定位优先 selection 的相邻单字符及折叠 range，再回退到同线程 system caret、`last_caret` cache 和无障碍自绘 caret；第一次 TSF 定位失败时先建立空 anchor 并再次查询，保留 layout/timer 重试等待宿主完成排版；仍不可用时仅候选面板使用有效编辑区的降级位置。anchor 只对原选区 range 的 Clone 执行 Collapse/StartComposition，不调用 SetText/SetSelection。专用 sink 用共享终止标记使旧句柄失效，不清 Rime；取消、提交、切换模式、失焦及停用均只 EndComposition，不清空 range，以免删除原选区文字。commit 在一个 write session 内临时 `StartComposition`、写入并 `EndComposition`。
...
候选窗缓存上次成功上传的完整 `CandidatePanelModel` 签名、位置、scale 和主题签名；四项相同即跳过字体栅格化与 layered-window 上传。每次 render 只取得一份解析后的 theme，panel 和 mode hint 辅助方法通过参数共享它，不再嵌套调用 `ThemeResolver::current()`。layout / caret retry 若解析位置与最后显示的 caret 相同，直接跳过整条 `update_ime_windows` 路径；真实状态更新仍进入变更检测。`CandidateUiManager` 使用独立的 `ui_state_changed`，包含 cursor、selection、分页元数据、select keys 和候选 comment，只有 UI 数据或 document manager 变化时才通知 `UpdateUIElement`。吞键路径的 `visible_state_changed` 只比较 preedit、候选 text / 数量、高亮和页码，不因 UI 元数据变化吞掉焦点切换后的首键或 solo Shift keyup；语言栏只有模式变化时才写 conversion compartment。隐藏窗口会取消 layout timer 并失效显示缓存。
...
- `current_mod_mask()` 额外上报 `RIME_MOD_LOCK`（CapsLock 的 toggle 位）和 `RIME_MOD_SUPER`（Win 键）。通用层 `key_policy::normalize_key_for_modifiers` 把 Lock modifier 折进字符 keysym 后剥掉该位；`VK_CAPITAL` 本身仍不送 librime，因此这不代表支持 schema 的 Caps Lock 切换策略。Win 键映射为 Super，用于 `key_policy::is_system_reserved_modifier` 的系统保留判定。
...
- `Shift_L` / `Shift_R` 只在 solo `OnKeyUp` 内依次以 press mask `0`、release mask `RIME_RELEASE_MASK` 送入 Rime。
...
诊断行时间戳精确到毫秒，统一带 `[pid:...][tid:...]`，并记录 input block reason、input-context 三项探测、password probe HRESULT、session reset、commit 丢失长度和 TSF edit-session 失败阶段。用户数据目录只解析一次，append log File 保存在进程级 `OnceLock` / `Mutex`，显式允许 `FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE`，避免常驻句柄阻止其他进程删除或轮转日志；每行可 flush 但不 fsync，不再逐行建目录、open / close。保留现有 `KEY_DIAGNOSTIC_LIMIT = 65_536`；所有按键与 commit 诊断只写长度和布尔状态，不写输入正文。

增加 `composition terminated identity_match={0|1} cleared={0|1}`、solo Shift 测试回调诊断及 `OnKeyUp shift keysym=... pending=... accepted=... ascii=...->...`。termination 实际清理时追加清零前的 `shift_pending={0|1}`，focus reset 同样记录 `focus reset cleared shift_pending={0|1}`，便于分辨 pending 被 focus / termination 清除、引擎未就绪和 Rime 已切换但未消费按键。engine-not-ready 日志使用实际 callback 名称。

F8 纯逻辑回归覆盖 mode-only apply、可见面板 layout 合并、每 apply 单次 ladder、相同上传签名跳过、语言栏只在 change 时写 compartment，以及 churn 不重置 probe 时间戳和三次同步拒绝后的退避。Windows `--tests` 检查只验证这些测试能够编译，不等同于已执行测试或 Windows / Chromium 性能验收。
