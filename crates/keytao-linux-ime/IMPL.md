...
当前系统输入法维护边界以 `crates/keytao-linux-ime` 为准。`src-tauri/src/ime/linux.rs` 仍保留旧的内嵌 Wayland IME/overlay 代码，但当前 `src-tauri/src/lib.rs` 没有把它作为系统输入法前端接入；不要在旧文件里继续扩展 Linux 系统输入法能力，除非先做明确迁移计划并删除或重新接入旧路径。
...
2. 清理或迁移 `src-tauri/src/ime/linux.rs` 旧路径，避免维护者误以为 GUI 内嵌 IME 仍是主路径。
3. 把可复用的按键 bypass、select key、Enter 提交、Shift release、candidate action 逻辑进一步收敛，减少各后端复制导致的行为漂移。
