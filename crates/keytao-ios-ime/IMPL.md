...
iOS 由于不能在键盘 view 外绘制候选窗，候选栏固定在键盘顶部。没有候选时显示空闲 toolbar：可滚动区包含功能面板、中英模式切换、选择/剪贴板/Emoji 面板和单手/分栏布局开关，主 toolbar 不放设置入口。Rime 选项页顶栏右侧依次为「编辑」图标和打开 App 的「设置」图标；「编辑」进入键盘内设置面板。系统键盘切换不在 toolbar 上，而是主键盘区强制注入的 🌐 键。
...
Rime 选项页顶栏的「编辑」图标会打开键盘内设置面板；设置面板的「返回」回到 Rime 选项页，右侧「设置」仍打开 App。可定制主工具栏默认置顶数为 5；读取或保存旧 `toolbarActionOrder` 时会剔除 `settings`。布局、候选字号、键角提示和反馈选项仍写入 `ios_ime.json`，配色与主题色仍写入 `theme.yaml`；没有新增配置存储。滑杆和色板在手势移动时只更新内存中的键盘配置、约束常量或主题预览，抬手时才完整应用布局，并通过 `persistSettings` 或主题写入器持久化一次。主题色色板末位是「自定义」色点（当前主题色非预设时显示该色并带选中环，否则显示彩虹环），点按进入面板内的自定义颜色子页：顶栏标题变为「自定义颜色」，「返回」等同「取消」。拖动色相条或明度饱和度方块时手势被独占，面板不滚动；预览行钉在面板底部，明度饱和度方块按面板高度收缩，「取消」「确定」不会被滚出可视区；抬手或手势被系统打断都保留拖到的颜色，只有「取消」「返回」才回滚；颜色实时预览按 `KeyTaoIMEInteractionTuning.colorPreviewThrottleMs`（50 ms）节流（首次立即生效，窗口结束与抬手时补最后一次），拖动过程中不写文件。「确定」通过主题写入器持久化一次 `ui.accentColor` 并回到设置列表；「取消」把打开子页时的主题色恢复到内存中。颜色换算集中在 `KeyTaoColorMath`（HSV ⇄ sRGB 与 `#RRGGBB`），由 `test-interaction-policy.sh` 覆盖往返、色相回绕与 s=0 / v=0 边界。高度预览不使用动画。
