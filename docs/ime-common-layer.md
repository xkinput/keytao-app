...
- Android 通过 `scripts/android-librime-runtime.sh` 导入 ABI 对应的 `librime.so` 闭包，Gradle 同步到 `jniLibs/<abi>`，基础 `rime-data` 同步到 APK assets 后由 `InputMethodService` 解包到 `filesDir/keytao/rime-data`。
...
- Linux daemon 每秒检查 stamp mtime，变化后调用 `ImeRuntime::reload_without_deploy()`，已有 `ImeRuntimeSession` 通过 generation 懒刷新。
- macOS IMK 在 `activateServer` 和每次 `handle` 前比较 stamp 内容，变化后调用无部署语义的 `keytao_reload()`；已有 `ImeRuntimeSession` 通过 generation 懒刷新。后续可继续把 stamp 检测也收敛到通用层。
- Windows TSF 在 focus 和 key event 热路径比较 `%APPDATA%/keytao/keytao-ime.reload` 的 mtime/size 签名，变化后异步重建只加载已部署产物的 runtime，并在 TSF edit session 中清理旧 composition 和候选窗。
- Android `InputMethodService` 在 input view 启动时比较 `filesDir/keytao/keytao-ime.reload` 的 mtime/size 签名，变化后调用 JNI 重新初始化，只加载已部署产物。
...
- Android input view 通过 JNI 调用 `keytao-theme` 解析 `filesDir/keytao/theme.yaml`，并消费 `CandidatePanelModel / ModeHintModel`；自绘 `View` 只负责把 resolved theme 和 model 映射到 Canvas。移动端键盘布局、hint 和滑动动作属于 Android adapter 配置，不进入 `ImeState`。
