...
- **改法**：候选栏布局改为「左侧固定一个 24–28dp 的工具栏折叠按钮 + 中间候选滚动区 + 右侧展开候选按钮」。折叠按钮点击后工具栏**覆盖**候选栏（再点或选中任一项即收回），不改变键盘高度。同时把中英态徽标放进折叠按钮（见 P2-11 合并实现，避免两处占宽）。
- **验收标准**：输入编码出现候选时，候选栏左侧仍有工具栏入口；点开可直接切中英、开剪贴板，且编码不丢失；收回后候选栏可用宽度与改造前差值 ≤ 30dp；两端一致。
...
- **改法**：把中英态徽标做进 P1-11 的候选栏左侧折叠按钮（按钮本身显示「中」/「EN」），任何时刻可见；工具栏补一个 SETTINGS 项（Android 直接接现成的绘制函数）。
- **验收标准**：输入过程中任意时刻都能看到当前中英态；误切到英文时可立即发现；工具栏点齿轮直达设置页。
