...
3. 每个 schema 都由 Manifest 中的 `:rime_deployer` Service 进程单独编译；结果通过 `ResultReceiver` 回到 App，进程随后退出，确保字典编译器的 native heap 真正归还系统。
