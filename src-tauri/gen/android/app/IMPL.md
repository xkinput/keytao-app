...
没有候选时，候选栏变成功能工具栏：左侧可滚动区提供结构化 `Rime` 页、中英切换、输入法地球键、`选择`、`剪贴板`、`Emoji`，主工具栏不放设置入口。地球键点击切换到下一个输入法，长按打开系统输入法列表；`选择` 直接打开均匀 5×5 editor 键盘层；`Rime` 页按“输入方案”和“选项”分组，选项从当前 schema 的 `switches` 动态读取并使用 schema 自带状态文案，无 switches 时显示空态。Rime 选项页顶栏右侧依次为「编辑」图标和打开 App 的「设置」图标；「编辑」进入键盘内设置面板。布尔项读取实际 option，单选组点击后循环切换，`ascii_mode` 仍通过 `setAsciiMode()` 更新。`剪贴板` 子页仍显示 Android 公开接口可读取的当前系统剪贴板和输入法会话内复制/剪切历史。符号页仍通过数字页 `#+=` 进入，符号页顶部工具栏显示 `中文` / `英文` tab，点击后通过 `setAsciiMode()` 同步 Rime 状态。
...
Rime 选项页顶栏的「编辑」图标会打开键盘内设置面板；设置面板的「返回」回到 Rime 选项页，右侧「设置」仍打开 App。可定制主工具栏默认置顶数为 5；读取或保存旧 `toolbarActionOrder` 时会剔除 `settings`。布局、候选字号、键角提示和反馈选项仍写入 `android_ime.json`，配色与主题色仍写入 `theme.yaml`；没有新增配置存储。滑杆和色板在手势移动时只更新内存中的键盘配置、尺寸或主题预览，抬手时才通过 `persistSettings` 或主题写入器持久化一次。主题色色板末位是「自定义」色点（当前主题色非预设时显示该色并带选中环，否则显示彩虹环），点按进入面板内的自定义颜色子页：顶栏标题变为「自定义颜色」，「返回」等同「取消」。拖动色相条或明度饱和度方块时手势被独占，面板不滚动；预览行钉在面板底部，明度饱和度方块按面板高度收缩，「取消」「确定」不会被滚出可视区；抬手或手势被系统打断都保留拖到的颜色，只有「取消」「返回」才回滚；颜色实时预览按 `KeytaoImeInteractionTuning.COLOR_PREVIEW_THROTTLE_MS`（50 ms）节流（首次立即生效，窗口结束与抬手时补最后一次），拖动过程中不写文件。「确定」通过主题写入器持久化一次 `ui.accentColor` 并回到设置列表；「取消」把打开子页时的主题色恢复到内存中。颜色换算集中在 `KeytaoColorMath`（HSV ⇄ sRGB 与 `#RRGGBB`），由 `KeytaoColorMathTest` 覆盖往返、色相回绕与 s=0 / v=0 边界。

### 设置面板布局规格

| 元素 | Android 尺寸（dp；文字为固定 sp） |
| --- | --- |
| 内容 | 单列，左右边距各 16 |
| 分组标题 | 高 28，字 12；除首组外组前留白 8 |
| 开关行 | 高 44，标签 14 且最多占行宽 60%；控件 52×28，状态字 12 |
| 三段配色 | 高 44；每段 56×28，字 12 |
| 滑杆行 | 高 56；文字基线 y=20，轨道中心 y=40、厚 4，滑块直径 16、描边 1 |
| 色板行 | 高 44；色点直径 24、间距 12，选中色环 2 |
| 页脚 | 行高 44；按钮高 36、字 13、间距 8 |
| 色相条（自定义颜色页） | 全宽 × 28，圆角 8，0–360° 横向色相渐变；游标宽 4、描边 1 |
| 明度饱和度方块 | 全宽 × min(140, 面板高 − 108)，圆角 8，白→当前色相横向 + 透明→黑纵向叠加；游标直径 14、描边 1 |
| 颜色预览行 | 高 44，钉在面板底部不随滚动；色块 44×28 圆角 8，十六进制文字 12 等宽；右侧「取消」「确定」按页脚按钮尺寸（高 36、字 13、间距 8） |
| 自定义颜色页行距 | 每行前留白 12（固定开销 12×3 + 28 + 44 = 108） |
| 分隔线 | 1，面板边框色 40% alpha |

所有设置面板文案都按单行绘制；标签测量后在可用宽度内尾部省略，不随 `candidateFontScale` 或候选标签字号缩放。
